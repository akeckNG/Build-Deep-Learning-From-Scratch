...
def cross_entropy_loss(logits, targets) -> "Tensor":
    ...
    targets = np.asarray(targets)  # accept lists as well as arrays
    if targets.ndim == 1:
        # this is an index of the right class in each batch entry
        indices = targets.astype(int)
        targets = np.zeros((indices.shape[0], logits.shape[1]))
        targets[np.arange(indices.shape[0]), indices] = 1.0

    lp = log_softmax(logits)
    return -(lp * targets).sum(axis=1).mean()
