...
class Tensor(Stage11_Tensor):
    ...
    def sum(self, axis: Optional[Union[int, Tuple[int, ...]]] = None,
        ...
        def _backward():
            ...
            if axis is not None and not keepdims:
                g = np.expand_dims(g, axis)  # restore dropped axes as size-1
            self.grad += g  # broadcasts back over the reduced axes
    ...
    def mean(self, axis: Optional[Union[int, Tuple[int, ...]]] = None,
        ...
        """
        out = self._make_tensor(np.mean(self.data, axis=axis, keepdims=keepdims), (self,), "mean")
        ...
        def _backward():
            ...
            if axis is not None and not keepdims:
                g = np.expand_dims(g, axis)  # restore dropped axes as size-1
            self.grad += g / N  # broadcasts back over the reduced axes
...
def cross_entropy_loss(logits, target) -> "Tensor":
    ...
    if isinstance(target, (int, np.integer)):
        # this is an index of the right class (accept numpy integer labels too)
        i = int(target)
